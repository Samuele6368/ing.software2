...
def test_full_enrollment_and_grading_flow(tmp_db):
    ...
    api.add_student("S999", "Anna", "Bianchi")
    api.enroll_student("S999", "INF01", "2025-01-15")
    api.record_grade("S999", "INF01", "2025-01-15", 30, False)

    transcript = api.get_transcript("S999")
...
def test_delete_course_cascades_registrations(tmp_db):
    ...
    api.add_student("S200", "Luca", "Verdi")
    api.enroll_student("S200", "MAT01", "2025-03-01")

    api.remove_course("MAT01")

    assert api.list_courses() == []
    assert api.list_exams() == []
    transcript = api.get_transcript("S200")
...
def test_delete_exam_removes_registrations(tmp_db):
    ...
    api.add_student("S300", "Giulia", "Neri")
    api.enroll_student("S300", "FIS01", "2025-04-20")

    api.remove_exam("FIS01", "2025-04-20")

    assert api.list_exams() == []
    transcript = api.get_transcript("S300")
    assert transcript["exams"] == []
...
def test_delete_grade_resets_values(tmp_db):
    ...
    api.add_student("S400", "Marta", "Blu")
    api.enroll_student("S400", "CHM01", "2025-05-15")
    api.record_grade("S400", "CHM01", "2025-05-15", 30, False)

    api.reset_grade("S400", "CHM01", "2025-05-15")

    transcript = api.get_transcript("S400")
